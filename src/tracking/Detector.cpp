	...
	for(index = indexes.begin(); index != indexes.end(); index++){
		if(*index < g_size){
			...
			if(!NN_TEST(conf)){ //Falsos negativos
				if(isin_p == -1 || isin_p == object_model[1].size() - 1) object_model[1].push_back(*sample);
				else object_model[1].insert(object_model[1].begin() + isin_p + 1, *sample); //Insere depois da amostra similar
